...
class GoodsReturnController extends Controller
{
    ...
    {
        return new GoodsIssueCollection(GoodsIssue::paginate(50));
    }
    ...
    {
        return GoodsIssueResource::collection(GoodsIssue::get());
    }

    public function dummyDoc(){
        $department = auth()->user()->employee->department->short_name;
        $dummy = \GenDocNumber::dummyDocnumber(env('DOC_GRT', 'GRT-NUM'), $department);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $issue->docstatus = 'A';
            $issue->doctype='R';
            $issue->cardcode = $request['card_code'];
            $issue->cardname = $request['card_name'];
            ...
            //$issue->total_price=$request['total_price'];
            $issue->total_price=0;
            $issue->quotation='';
            $issue->printed='N';
            $issue->userid=$u->id;;
            ...
            $issue->docnum = \GenDocNumber::genDocnumber(env('DOC_GRT', 'GRT-NUM'), $department);
            $issue->save();
