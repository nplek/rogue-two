    ...
    <div class="col-xs-12">
        <div class="box box-primary">
            ...
            <form v-on:submit="saveForm()" >
                <div class="box-body">
                    <div class="row">
                        <div class="col-xs-4 form-group" :class="{ 'has-error': $v.returnitem.warehouse.$error }">
                            <label class="control-label">Warehouse code</label>
                            <multiselect 
                                v-model.trim="$v.returnitem.warehouse.$model"
                            ...
                            <div class="help-block" v-if="!$v.returnitem.warehouse.required">Field is required</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-4 form-group" >
                            <label class="control-label">Doc#</label>
                            <input type="text" v-model="returnitem.docnum" class="form-control" disabled>
                        </div>
                        <div class="col-xs-4 form-group" :class="{ 'has-error': $v.returnitem.docdate.$error }">
                            <label class="control-label">Doc date</label>
                            <date-picker v-model.trim="$v.returnitem.docdate.$model" :config="{format: 'DD/MM/YYYY'}" disabled></date-picker>
                            <div class="help-block" v-if="!$v.returnitem.docdate.required">Field is required</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-4 form-group" :class="{ 'has-error': $v.returnitem.shipdate.$error }">
                            <label class="control-label">Issue date</label>
                            <date-picker v-model.trim="$v.returnitem.shipdate.$model" :config="{format: 'DD/MM/YYYY'}"></date-picker>
                            <div class="help-block" v-if="!$v.returnitem.shipdate.required">Field is required</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-8 form-group" :class="{ 'has-error': $v.returnitem.remark.$error }">
                            <label class="control-label">Remark</label>
                            <input type="text" v-model.trim="$v.returnitem.remark.$model" class="form-control">
                            <div class="help-block" v-if="!$v.returnitem.remark.maxLength">Field must have at most {{ $v.returnitem.remark.$params.maxLength.max }} letters.</div>
                        </div>
                    </div>
                    <hr />
                    <h3> Transactions </h3>
                    <div class="form-group" :class="{ 'has-error': $v.returnitem.transactions.$error }">
                        <label for="">Add Transaction:</label>
                        <button v-if="returnitem.whs_id" type="button" class="btn btn-primary" data-toggle="modal" data-target="#transactionModal">
                            +
                        </button>
                        <div class="help-block" v-if="!$v.returnitem.transactions.required">Transaction is required</div>
                        <!-- Modal -->
                    ...
                    <table class="table">
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td></td>
                                <td>Total</td>
                                <td><b>{{ returnitem.total_qty | toQty}}</b></td>
                                <td></td>
                                <td></td>
                            </tr>
                        </tfoot>
    ...
    export default {
        ...
        methods: {
            ...
            businesspartnerChange(value,id){
                var app = this;
                app.returnitem.card_id = value.id;
                app.returnitem.card_code = value.card_code;
                app.returnitem.card_name = value.card_name;
            },
            ...
            warehouseChange(value,id){
                var app = this;
                app.returnitem.whs_id = value.id;
                app.returnitem.whs_code = value.whs_code;
                app.returnitem.transactions=[];
                this.getItemsList();
            },
            ...
            getDocNum(){
                let app = this;
                axios.post('/api/returnitems/docnum',null,{
                        headers: {
                    ...
                    .then(function (resp) {
                        app.returnitem.docnum = resp.data.data.docnum;
                    })
                    .catch(function () {
            ...
            getItemsList(){
                let app = this;
                var whs_id = app.returnitem.whs_id;
            ...
            saveForm() {
                ...
                var app = this;
                var newGoods = app.returnitem;
                newGoods.transactions = app.transactions;
                app.$v.$touch();
                if (app.$v.$invalid){
                    return;
                } else {
                    axios.post('api/returnitems', newGoods,{
                        headers: {
                            'Accept': 'application/json',
                            'Authorization': 'Bearer ' + app.token
                        }
            ...
            calcTotal(){
                ...
                this.returnitem.total_qty = q_total;
            },
            updateTransaction(){
                var app = this;
                app.nextTxnId = 1;
