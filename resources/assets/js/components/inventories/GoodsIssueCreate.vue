    ...
    <div class="col-xs-12">
        <div class="box box-primary">
            ...
            <form v-on:submit="saveForm()" >
                <div class="box-body">
                    <div class="row">
                        <div class="col-xs-4 form-group" :class="{ 'has-error': $v.goodsissue.warehouse.$error }">
                            <label class="control-label">Warehouse code</label>
                            <multiselect 
                                v-model.trim="$v.goodsissue.warehouse.$model"
                            ...
                            <div class="help-block" v-if="!$v.goodsissue.warehouse.required">Field is required</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-4 form-group" >
                            <label class="control-label">Doc#</label>
                            <input type="text" v-model="goodsissue.docnum" class="form-control" disabled>
                        </div>
                        <div class="col-xs-4 form-group" :class="{ 'has-error': $v.goodsissue.docdate.$error }">
                            <label class="control-label">Doc date</label>
                            <date-picker v-model.trim="$v.goodsissue.docdate.$model" :config="{format: 'DD/MM/YYYY'}" disabled></date-picker>
                            <div class="help-block" v-if="!$v.goodsissue.docdate.required">Field is required</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-4 form-group" :class="{ 'has-error': $v.goodsissue.issuedate.$error }">
                            <label class="control-label">Issue date</label>
                            <date-picker v-model.trim="$v.goodsissue.issuedate.$model" :config="{format: 'DD/MM/YYYY'}"></date-picker>
                            <div class="help-block" v-if="!$v.goodsissue.issuedate.required">Field is required</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-8 form-group" :class="{ 'has-error': $v.goodsissue.remark.$error }">
                            <label class="control-label">Remark</label>
                            <input type="text" v-model.trim="$v.goodsissue.remark.$model" class="form-control">
                            <div class="help-block" v-if="!$v.goodsissue.remark.maxLength">Field must have at most {{ $v.goodsissue.remark.$params.maxLength.max }} letters.</div>
                        </div>
                    </div>
                    <hr />
                    <h3> Transactions </h3>
                    <div class="form-group" :class="{ 'has-error': $v.goodsissue.transactions.$error }">
                        <label for="">Add Transaction:</label>
                        <button v-if="goodsissue.whs_id" type="button" class="btn btn-primary" data-toggle="modal" data-target="#transactionModal">
                            +
                        </button>
                        <div class="help-block" v-if="!$v.goodsissue.transactions.required">Transaction is required</div>
                        <!-- Modal -->
                    ...
                    <table class="table">
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td></td>
                                <td>Total</td>
                                <td><b>{{ goodsissue.total_qty | toQty}}</b></td>
                                <td></td>
                                <td></td>
                            </tr>
                        </tfoot>
    ...
    export default {
        ...
        methods: {
            ...
            warehouseChange(value,id){
                var app = this;
                app.goodsissue.whs_id = value.id;
                app.goodsissue.whs_code = value.whs_code;
                app.goodsissue.transactions=[];
                this.getItemsList();
            },
            ...
            getDocNum(){
                let app = this;
                axios.post('/api/goodsissues/docnum',null,{
                        headers: {
                    ...
                    .then(function (resp) {
                        app.goodsissue.docnum = resp.data.data.docnum;
                    })
                    .catch(function () {
            ...
            getItemsList(){
                let app = this;
                var whs_id = app.goodsissue.whs_id;
            ...
            saveForm() {
                ...
                var app = this;
                var newGoods = app.goodsissue;
                newGoods.transactions = app.transactions;
                app.$v.$touch();
                if (app.$v.$invalid){
                    return;
                } else {
                    axios.post('/api/goodsissues', newGoods,{
                        headers: {
                            'Accept': 'application/json',
                            'Authorization': 'Bearer ' + app.token
                        }
            ...
            calcTotal(){
                ...
                this.goodsissue.total_qty = q_total;
            },
            updateTransaction(){
                var app = this;
                app.nextTxnId = 1;
