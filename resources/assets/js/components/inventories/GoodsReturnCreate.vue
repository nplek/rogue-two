    ...
    <div class="col-xs-12">
        <div class="box box-primary">
            ...
            <form v-on:submit="saveForm()" >
                <div class="box-body">
                    <div class="row">
                        <div class="col-xs-4 form-group" :class="{ 'has-error': $v.goodsreturn.warehouse.$error }">
                            <label class="control-label">Warehouse code</label>
                            <multiselect 
                                v-model.trim="$v.goodsreturn.warehouse.$model"
                            ...
                            <div class="help-block" v-if="!$v.goodsreturn.warehouse.required">Field is required</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-4 form-group" >
                            <label class="control-label">Doc#</label>
                            <input type="text" v-model="goodsreturn.docnum" class="form-control" disabled>
                        </div>
                        <div class="col-xs-4 form-group" :class="{ 'has-error': $v.goodsreturn.docdate.$error }">
                            <label class="control-label">Doc date</label>
                            <date-picker v-model.trim="$v.goodsreturn.docdate.$model" :config="{format: 'DD/MM/YYYY'}" disabled></date-picker>
                            <div class="help-block" v-if="!$v.goodsreturn.docdate.required">Field is required</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-4 form-group" :class="{ 'has-error': $v.goodsreturn.issuedate.$error }">
                            <label class="control-label">Issue date</label>
                            <date-picker v-model.trim="$v.goodsreturn.issuedate.$model" :config="{format: 'DD/MM/YYYY'}"></date-picker>
                            <div class="help-block" v-if="!$v.goodsreturn.issuedate.required">Field is required</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-4 form-group" :class="{ 'has-error': $v.goodsreturn.cardcode.$error }">
                            <label class="control-label">Supplier code</label>
                            <multiselect 
                                v-model.trim="$v.goodsreturn.cardcode.$model"
                            ...
                            <div class="help-block" v-if="!$v.goodsreturn.cardcode.required">Field is required</div>
                        </div>
                        <div class="col-xs-4 form-group" :class="{ 'has-error': $v.goodsreturn.ref1.$error }">
                            <label class="control-label">Ref PO#</label>
                            <input type="text" v-model.trim="$v.goodsreturn.ref1.$model" class="form-control">
                            <div class="help-block" v-if="!$v.goodsreturn.ref1.required">Field is required</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-8 form-group" :class="{ 'has-error': $v.goodsreturn.remark.$error }">
                            <label class="control-label">Remark</label>
                            <input type="text" v-model.trim="$v.goodsreturn.remark.$model" class="form-control">
                            <div class="help-block" v-if="!$v.goodsreturn.remark.maxLength">Field must have at most {{ $v.goodsreturn.remark.$params.maxLength.max }} letters.</div>
                        </div>
                    </div>
                    <hr />
                    <h3> Transactions </h3>
                    <div class="form-group" :class="{ 'has-error': $v.goodsreturn.transactions.$error }">
                        <label for="">Add Transaction:</label>
                        <button v-if="goodsreturn.whs_id" type="button" class="btn btn-primary" data-toggle="modal" data-target="#transactionModal">
                            +
                        </button>
                        <div class="help-block" v-if="!$v.goodsreturn.transactions.required">Transaction is required</div>
                        <!-- Modal -->
                    ...
                    <table class="table">
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td></td>
                                <td>Total</td>
                                <td><b>{{ goodsreturn.total_qty | toQty}}</b></td>
                                <td></td>
                                <td></td>
                            </tr>
                        </tfoot>
    ...
    export default {
        ...
        methods: {
            ...
            businesspartnerChange(value,id){
                var app = this;
                app.goodsreturn.card_id = value.id;
                app.goodsreturn.card_code = value.card_code;
                app.goodsreturn.card_name = value.card_name;
            },
            ...
            warehouseChange(value,id){
                var app = this;
                app.goodsreturn.whs_id = value.id;
                app.goodsreturn.whs_code = value.whs_code;
                app.goodsreturn.transactions=[];
                this.getItemsList();
            },
            ...
            getDocNum(){
                let app = this;
                axios.post('/api/goodsreturns/docnum',null,{
                        headers: {
                    ...
                    .then(function (resp) {
                        app.goodsreturn.docnum = resp.data.data.docnum;
                    })
                    .catch(function () {
            ...
            getItemsList(){
                let app = this;
                var whs_id = app.goodsreturn.whs_id;
            ...
            saveForm() {
                ...
                var app = this;
                var newGoods = app.goodsreturn;
                newGoods.transactions = app.transactions;
                app.$v.$touch();
                if (app.$v.$invalid){
                    return;
                } else {
                    axios.post('/api/goodsreturns', newGoods,{
                        headers: {
                            'Accept': 'application/json',
                            'Authorization': 'Bearer ' + app.token
                        }
            ...
            calcTotal(){
                ...
                this.goodsreturn.total_qty = q_total;
            },
            updateTransaction(){
                var app = this;
                app.nextTxnId = 1;
