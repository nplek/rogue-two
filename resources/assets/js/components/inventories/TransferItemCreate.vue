    ...
    <div class="col-xs-12">
        <div class="box box-primary">
            ...
            <form v-on:submit="saveForm()" >
                <div class="box-body">
                    <div class="row">
                        <div class="col-xs-4 form-group" :class="{ 'has-error': $v.tranitem.warehouse.$error }">
                            <label class="control-label">Form Warehouse</label>
                            <multiselect 
                                v-model.trim="$v.tranitem.warehouse.$model"
                            ...
                            <div class="help-block" v-if="!$v.tranitem.warehouse.required">Field is required</div>
                        </div>
                        <div class="col-xs-4 form-group" :class="{ 'has-error': $v.tranitem.towarehouse.$error }">
                            <label class="control-label">To Warehouse</label>
                            <multiselect 
                                v-model.trim="$v.tranitem.towarehouse.$model"
                                @input="towarehouseChange"
                                :options="whs" 
                                :custom-label="warehouseWithFullName" 
                                placeholder="Please select" 
                                label="name" 
                            ...
                            <div class="help-block" v-if="!$v.tranitem.towarehouse.required">Field is required</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-4 form-group" >
                            <label class="control-label">Doc#</label>
                            <input type="text" v-model="tranitem.docnum" class="form-control" disabled>
                        </div>
                        <div class="col-xs-4 form-group" :class="{ 'has-error': $v.tranitem.docdate.$error }">
                            <label class="control-label">Doc date</label>
                            <date-picker v-model.trim="$v.tranitem.docdate.$model" :config="{format: 'DD/MM/YYYY'}" disabled></date-picker>
                            <div class="help-block" v-if="!$v.tranitem.docdate.required">Field is required</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-4 form-group" :class="{ 'has-error': $v.tranitem.shipdate.$error }">
                            <label class="control-label">Issue date</label>
                            <date-picker v-model.trim="$v.tranitem.shipdate.$model" :config="{format: 'DD/MM/YYYY'}"></date-picker>
                            <div class="help-block" v-if="!$v.tranitem.shipdate.required">Field is required</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-8 form-group" :class="{ 'has-error': $v.tranitem.remark.$error }">
                            <label class="control-label">Remark</label>
                            <input type="text" v-model.trim="$v.tranitem.remark.$model" class="form-control">
                            <div class="help-block" v-if="!$v.tranitem.remark.maxLength">Field must have at most {{ $v.tranitem.remark.$params.maxLength.max }} letters.</div>
                        </div>
                    </div>
                    <hr />
                    <h3> Transactions </h3>
                    <div class="form-group" :class="{ 'has-error': $v.tranitem.transactions.$error }">
                        <label for="">Add Transaction:</label>
                        <button v-if="tranitem.whs_id" type="button" class="btn btn-primary" data-toggle="modal" data-target="#transactionModal">
                            +
                        </button>
                        <div class="help-block" v-if="!$v.tranitem.transactions.required">Transaction is required</div>
                        <!-- Modal -->
                    ...
                    <table class="table">
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td></td>
                                <td>Total</td>
                                <td><b>{{ tranitem.total_qty | toQty}}</b></td>
                                <td></td>
                                <td></td>
                            </tr>
                        </tfoot>
    ...
    export default {
        ...
        methods: {
            ...
            warehouseChange(value,id){
                var app = this;
                app.tranitem.whs_id = value.id;
                app.tranitem.whs_code = value.whs_code;
                app.tranitem.transactions=[];
                this.getItemsList();
            },
            towarehouseChange(value,id){
                var app = this;
                app.tranitem.towhs_id = value.id;
                app.tranitem.towhs_code = value.whs_code;
            },
            ...
            getDocNum(){
                let app = this;
                axios.post('/api/tranitems/docnum',null,{
                        headers: {
                    ...
                    .then(function (resp) {
                        app.tranitem.docnum = resp.data.data.docnum;
                    })
                    .catch(function () {
            ...
            getItemsList(){
                let app = this;
                var whs_id = app.tranitem.whs_id;
            ...
            saveForm() {
                ...
                var app = this;
                var newGoods = app.tranitem;
                newGoods.transactions = app.transactions;
                app.$v.$touch();
                if (app.$v.$invalid){
                    return;
                } else {
                    axios.post('/api/tranitems', newGoods,{
                        headers: {
                            'Accept': 'application/json',
                            'Authorization': 'Bearer ' + app.token
                        }
            ...
            calcTotal(){
                ...
                this.tranitem.total_qty = q_total;
            },
            updateTransaction(){
                var app = this;
                app.nextTxnId = 1;
