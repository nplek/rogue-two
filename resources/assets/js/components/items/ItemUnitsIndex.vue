<template>
    <div class="row">
        <div class="form-group">
            <router-link v-if="auth.can.create" :to="{name: 'createItemUnit'}" class="btn btn-success">New</router-link>
        </div>
                ...
                <table class="table table-bordered table-striped">
                    ...
                    <tr v-for="(item, index) in items" v-bind:key="item.id">
                        ...
                        <td>
                            ...
                            <router-link :to="{name: 'editItemUnit', params: {id: item.id}}" class="btn btn-sm btn-info">
                                Edit
                            </router-link>
        ...
        methods: {
            ...
            deleteEntry(id, index) {
                if (confirm("Do you really want to delete it?")) {
                    ...
                    axios.delete('/api/itemunits/' + id,{
                        ...
                        .catch(function (resp) {
                            alert("Could not delete itemunits" );
                        });
                }
            },
            restoreEntry(id,index) {
                if (confirm("Do you really want to restore it?")) {
                    var app = this;
                    axios.post('/api/itemunits/' + id + '/restore',null,{
                            headers: {
                                'Accept': 'application/json',
                                'Authorization': 'Bearer '+ app.token
                            }
                        ...
                        .catch(function (resp) {
                            alert("Could not restore itemunits");
                        });
                }
            }
        }
